...
public class ServerManager extends Thread {
	 ...
	 * 
	 * @param port
	 *            port on which the manager will listen for users
	 * @param blockingQueue
	 *            queue with events
	 * @throws IOException
	 *             when ServerSocket cannot be created on selected port
	 */
	public ServerManager(int port, BlockingQueue<BaseEvent> blockingQueue) throws IOException {
		this.serverSocket = new ServerSocket(port);
		this.blockingQueue = blockingQueue;
		clients = new HashSet<ClientManager>();
	}
