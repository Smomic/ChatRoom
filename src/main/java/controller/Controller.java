...
public class Controller {
     ...
     * @param serverManager manager used by this controller to send messages over network
     * @param blockingQueue blockingQueue from which events are read
     */
    public Controller(Model model, ServerManager serverManager, BlockingQueue<BaseEvent> blockingQueue) {
        ...
        clientToUserNameMap = new HashMap<>();
    ...
    public void start() {
        while (true) {
            ...
            getStrategy(event).execute(event);
        }
    }

    private ServerStrategy getStrategy(BaseEvent event) {
        return eventsToStrategyMap.get(event.getClass());
    }
    ...
    private abstract class ServerStrategy {
        ...
        void sendBroadcast(UserStatus status) {
